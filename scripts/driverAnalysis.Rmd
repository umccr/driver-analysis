...
for ( i in 1:length(mafFiles) ) {
  if ( length(signif_genes[[i]]) > 0 ) {
    ...
    if ( nrow(signif_genes.excl[[i]]) > 0 ) {
      cat("* The genomic location(s) for the following detected cancer driver gene(s) are not defined and thus are not presented on the circos plot:*", paste(signif_genes.excl[[i]], collapse = ", "), "*\n\n", sep=" ")
    }
