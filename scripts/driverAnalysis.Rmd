...
for ( i in 1:length(mafFiles) ) {
  mafInfo[[i]] <- maftools::read.maf(maf = mafFiles[i], removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE)
}
...
if ( !is.na(params$samples_blacklist) ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    mafInfo[[i]] <- maftools::read.maf(maf = mafInfo[[i]], removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE)
  }
}
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Order the table by P_VALUE, MUTS, MUTS_RECURRENCE and SAMPLES
  oncodriveFML.res.table[[i]] <- oncodriveFML.res.table[[i]][ order(oncodriveFML.res.table[[i]]$P_VALUE), ]
  oncodriveFML.res.table[[i]] <- oncodriveFML.res.table[[i]][ order( oncodriveFML.res.table[[i]]$MUTS, oncodriveFML.res.table[[i]]$MUTS_RECURRENCE, oncodriveFML.res.table[[i]]$SAMPLES, decreasing = TRUE), ]
