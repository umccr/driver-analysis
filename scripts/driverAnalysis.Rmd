...
* **[ActiveDriverWGS](https://github.com/reimandlab/ActiveDriverWGSR){target="_blank"}** (see paper by [Zhu *et al*., 2020](https://pubmed.ncbi.nlm.nih.gov/31954095){target="_blank"} for details - WIP)
* **[OncodriveClustl](https://bitbucket.org/bbglab/oncodriveclustl/src/master/){target="_blank"}** (see paper by [ Nuria Lopez-Bigas *et al*., 2019](https://www.ncbi.nlm.nih.gov/pubmed/23884480){target="_blank"} for details)
...
* **[MutPanning](https://www.genepattern.org/modules/docs/MutPanning#gsc.tab=0){target="_blank"}**  (see paper by [Felix *et al*., 2020](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7031046/){target="_blank"} for details - WIP)
* **[CHASMPlus](https://chasmplus.readthedocs.io/en/latest/quickstart_opencravat.html#interpretation){target="_blank"}**  (see paper by [Collin Tokheim *et al*., 2019](https://pubmed.ncbi.nlm.nih.gov/31202631/){target="_blank"} for details)
...
* **runactivedriverwgs**: `r params$runactivedriverwgs`
  ...
  for ( dataset in datasets ) {
    ...
    if ( nrow(mut) != 0 ) {
      ...
      ##### ...otherwise leave a message
        ...
        ##### Check if the genes has any synonymous variants
...
if ( !is.na(params$samples_id_cols) ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    ##### Check if user-defined column name exists
    if ( samples_id_cols[i] %in% names(mafInfo[[i]]@data) && samples_id_cols[i] != "Tumor_Sample_Barcode" ) {
      ...
      ##### Now read the data with changed column names as a maf object
...
##### Read in ChasmPLUS analysis results (https://chasmplus.readthedocs.io/en/latest/quickstart_opencravat.html)
runChasmplusChunk <- FALSE

if ( params$chasmplus != "none" ){
  
  chasmPLUS.res <- unlist(strsplit(params$chasmplus, split=',', fixed=TRUE))
  
  chasmPLUS.res.table <- vector("list", length(mafFiles))
  names(chasmPLUS.res.table) <- datasets.list
  
  for ( i in 1:length(mafFiles) ) {
    
    ##### Read in the results table
    if ( file.exists(paste0(chasmPLUS.res[i], ".tsv")) ) {
      
      chasmPLUS.res.table[[i]] <- read.table(paste0(chasmPLUS.res[i], ".tsv"), sep="\t", as.is=FALSE, header=TRUE, row.names=NULL, skip = 6)
      
      runChasmplusChunk <- TRUE
    }
  }
}
...
##### Read in OncodriveCLUSTL analysis results (https://bitbucket.org/bbglab/oncodriveclustl/src/master/)
runOncodriveClustlChunk <- FALSE

if ( params$oncodriveclustl != "none" ){
  
  oncodriveCLUSTL.res <- unlist(strsplit(params$oncodriveclustl, split=',', fixed=TRUE))
  
  oncodriveCLUSTL.res.table <- vector("list", length(mafFiles))
  names(oncodriveCLUSTL.res.table) <- datasets.list
  
  for ( i in 1:length(mafFiles) ) {
    
    ##### Read in the results table
    if ( file.exists(paste0(oncodriveCLUSTL.res[i], ".txt")) ) {
      
      oncodriveCLUSTL.res.table[[i]] <- read.table(paste0(oncodriveCLUSTL.res[i], ".txt"), sep="\t", as.is=FALSE, header=TRUE, row.names=NULL)
      
      runOncodriveClustlChunk <- TRUE
    }
  }
}
...
##### Initiate a list to record significant driver genes for each dataset
...
##### Initiate a list to record significant driver genes for each method
methods <- c("dNdScv", "dNdSloc")

if ( runChasmplusChunk ) {
  methods <- c(methods, "ChasmPLUS")
}
...
if ( runOncodriveClustlChunk ) {
  methods <- c(methods, "OncodriveCLUSTL")
}

if ( params$runactivedriverwgs ) {
  methods <- c(methods, "ActiveDriverWGS")
  runActiveDriverWGSChunk <- TRUE
} else {
  runActiveDriverWGSChunk <- FALSE
...
##### Exclude user-defined samples(s) from the analysis
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Identify samples with no non-synonymous variants according to corresponding MAF file
  ...
  ##### Check if there there are any samples with no non-synonymous variants. If so, add them to data frame
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Run analysis for all genes - peek into specified params at https://rdrr.io/github/im3sanger/dndscv/src/R/dndscv.R
  } else {
...
theta_indels <- vector("list", length(mafFiles))
names(theta_indels) <- datasets.list
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Calculate theta estimates for substitutions and indels
  theta[[i]] <- round(dndsout[[i]]$nbreg$theta, digits = 2)
  theta_indels[[i]] <- round(dndsout[[i]]$nbregind$theta, digits = 2)
  ...
  if ( params$ratios_ci ) {
    ...
    if ( dNdS_cov ) {
      ...
      signif_genes_table.datasets[[i]]$dNdScv <- signif_genes_table.datasets[[i]]$dNdScv[ , c("gene_name", "pglobal_cv", "qglobal_cv", "pallsubs_cv", "qallsubs_cv", "n_syn", "n_mis", "n_non", "n_spl", "wmis_cv", "pmis_cv",  "wnon_cv", "wspl_cv", "ptrunc_cv")]
    }
  }
  ...
  #### Record significant genes and corresponding p-values (this will be used in summary)
  signif_genes_p <- vector("list", length(mafFiles))
  names(signif_genes_p) <- datasets.list
  signif_genes_p[[i]]$dndscv <- signif_genes_table.datasets[[i]]$dNdScv[c("gene_name", "pglobal_cv")] %>%
  dplyr::rename(p_dndscv = pglobal_cv)
  signif_genes_p[[i]]$dndsloc <- signif_genes_table.datasets[[i]]$dNdSloc[c("gene_name", "pall_loc")] %>%
  dplyr::rename(p_dndsloc = pall_loc)
}
```
...
```{r ActiveDriverWGS_format_data, comment = NA, message=FALSE, warning=FALSE, eval = runActiveDriverWGSChunk}
...
for ( i in 1:length(mafFiles) ) {
  ...
  ActiveDriverWGSInfo[[i]] <- rbind(mafInfo[[i]]@data[, c("Hugo_Symbol", "Chromosome", "Start_Position", "End_Position", "Reference_Allele", "Tumor_Seq_Allele2", "Tumor_Sample_Barcode")], mafInfo[[i]]@maf.silent[, c("Hugo_Symbol", "Chromosome", "Start_Position", "End_Position", "Reference_Allele", "Tumor_Seq_Allele2", "Tumor_Sample_Barcode")])
  ...
  ##### Load UMCCR cancer gene list for ActiveDriverWGS if the "activedriverwgs_all_genes" parameter is not set to TRUE
  ...
  ##### Make sure that all rows have defined "ref" and "alt" values (none NAs)
  ActiveDriverWGSInfo[[i]] <- ActiveDriverWGSInfo[[i]][complete.cases(ActiveDriverWGSInfo[[i]]), ]
  
  #### Make sure Reference and alternate alleles are A, T, C, G or -
  ActiveDriverWGSInfo[[i]] <- ActiveDriverWGSInfo[[i]][grepl("[ATGC\\-]", ref),]
  ActiveDriverWGSInfo[[i]] <- ActiveDriverWGSInfo[[i]][grepl("[ATGC\\-]", alt),]
  
  #### Make sure patient column is string
  ActiveDriverWGSInfo[[i]]$patient <- as.character(ActiveDriverWGSInfo[[i]]$patient)
...
```{r ActiveDriverWGS_analysis, comment = NA, message=FALSE, warning=FALSE, eval=runActiveDriverWGSChunk }
...
for ( i in 1:length(mafFiles) ) {
  ...
  sink("/dev/null")
  ##### NOTE: the analysis crash when "recovery.dir" parameter (for storing recovery files) is used so currently it's set to NULL. Once fixed it can be set to "paste(outDir, "ActiveDriverWGS_recovery", sep="/")"
  ...
  #### Record significant genes and corresponding p-values (this will be used in summary)
  signif_genes_p[[i]]$activedriverwgs <- signif_genes_table.datasets[[i]]$ActiveDriverWGS[c("id", "pp_element")] %>%
  dplyr::rename(gene_name = id, p_activedriverwgs = pp_element)
}
...
```{r chasmPLUS_load_results,  include=FALSE, eval=runChasmplusChunk}
##### Create a list to store ChasmPLUS info for individual datasets
signif_genes.methods$ChasmPLUS <- vector("list", length(datasets.list))
names(signif_genes.methods$ChasmPLUS) <- datasets.list

###### Record results for each dataset
for ( i in 1:length(mafFiles) ) {
    
  ##### Order the table by Gene_p.value
  chasmPLUS.res.table[[i]] <- chasmPLUS.res.table[[i]][ order(chasmPLUS.res.table[[i]]$Gene_p.value), ]
  rownames(chasmPLUS.res.table[[i]]) = NULL
  
  ##### Calculate expected p-value
  chasmPLUS.res.table[[i]]$P_value_exp <- ppoints(length(chasmPLUS.res.table[[i]]$Gene_p.value))
  
  ##### Calculate q-value from p-value. Using lambda 0 as we have limited set of p-values https://support.bioconductor.org/p/105623/
  chasmPLUS.res.table[[i]]$Q_value <- qvalue(chasmPLUS.res.table[[i]]$Gene_p.value, lambda = 0)$qvalues

  ##### Record significant driver genes for each dataset
  signif_genes.datasets[[i]]$ChasmPLUS <- unique(c(as.character(chasmPLUS.res.table[[i]]$Gene)[ chasmPLUS.res.table[[i]]$Gene_p.value <= params$chasmplus_p  ], as.character(chasmPLUS.res.table[[i]]$Gene)[ chasmPLUS.res.table[[i]]$Q_VALUE <= params$chasmplus_q ]))
  
  ##### Remove NAs
  signif_genes.datasets[[i]]$ChasmPLUS <- signif_genes.datasets[[i]]$ChasmPLUS[ !is.na(signif_genes.datasets[[i]]$ChasmPLUS)]
  
  ##### Report significant driver genes
  signif_genes_table.datasets[[i]]$ChasmPLUS <- chasmPLUS.res.table[[i]][ chasmPLUS.res.table[[i]]$Gene %in%  signif_genes.datasets[[i]]$ChasmPLUS , ]
  rownames(signif_genes_table.datasets[[i]]$ChasmPLUS) = NULL
  
  ##### Record significant driver genes for each dataset and method
  signif_genes.methods$ChasmPLUS[[i]] <- as.vector(signif_genes_table.datasets[[i]]$ChasmPLUS$Gene)
  
  ##### Organise the table of significant genes
  signif_genes_table.datasets[[i]]$ChasmPLUS <- signif_genes_table.datasets[[i]]$ChasmPLUS[ , c("Gene", "Gene_p.value", "P_value_exp", "Q_value", "Number_of.Coding.Variants", "Number_of.Noncoding.Variants", "Sequence_Ontology", "All_Sequence.Ontologies")]
  
  #### Record significant genes and corresponding p-values (this will be used in summary)
  signif_genes_p[[i]]$ChasmPLUS <- signif_genes_table.datasets[[i]]$ChasmPLUS[c("Gene", "Gene_p.value")] %>%
  dplyr::rename(gene_name = Gene, p_chasmplusl = Gene_p.value)
}
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  oncodriveFML.res.table[[i]] <- oncodriveFML.res.table[[i]] %>%
  ...
  #### Record significant genes and corresponding p-values (this will be used in summary)
  signif_genes_p[[i]]$oncodrivefml <- signif_genes_table.datasets[[i]]$OncodriveFML[c("SYMBOL", "P_VALUE")] %>%
  dplyr::rename(gene_name = SYMBOL, p_oncodrivefml = P_VALUE)
}
```

```{r oncodriveCLUSTL_load_results,  include=FALSE, eval=runOncodriveClustlChunk}
##### Create a list to store OncodriveCLUSTL info for individual datasets
signif_genes.methods$OncodriveCLUSTL <- vector("list", length(datasets.list))
names(signif_genes.methods$OncodriveCLUSTL) <- datasets.list

###### Record results for each dataset
for ( i in 1:length(mafFiles) ) {
  #### Filter rows where SCORE is NA or <0
  oncodriveCLUSTL.res.table[[i]] <- oncodriveCLUSTL.res.table[[i]][ !is.na(oncodriveCLUSTL.res.table[[i]]$SCORE) & oncodriveCLUSTL.res.table[[i]]$SCORE > 0, ]
  
  ##### Order the table by SCORE and P-value
  oncodriveCLUSTL.res.table[[i]] <- oncodriveCLUSTL.res.table[[i]][ order( oncodriveCLUSTL.res.table[[i]]$SCORE, decreasing = TRUE), ]
  oncodriveCLUSTL.res.table[[i]] <- oncodriveCLUSTL.res.table[[i]][ order(oncodriveCLUSTL.res.table[[i]]$P_ANALYTICAL), ]
  rownames(oncodriveCLUSTL.res.table[[i]]) = NULL
  
  ##### Calculate expected p-value
  oncodriveCLUSTL.res.table[[i]]$P_VALUE_EXP <- ppoints(length(oncodriveCLUSTL.res.table[[i]]$P_ANALYTICAL))

  ##### Record significant driver genes for each dataset
  signif_genes.datasets[[i]]$OncodriveCLUSTL <- as.character(oncodriveCLUSTL.res.table[[i]][
    oncodriveCLUSTL.res.table[[i]]$P_ANALYTICAL <= params$oncodriveclustl_p &
      oncodriveCLUSTL.res.table[[i]]$Q_ANALYTICAL <= params$oncodriveclustl_q, "SYMBOL"])
  
  ##### Remove NAs
  signif_genes.datasets[[i]]$OncodriveCLUSTL <- signif_genes.datasets[[i]]$OncodriveCLUSTL[ !is.na(signif_genes.datasets[[i]]$OncodriveCLUSTL)]
  
  ##### Report significant driver genes
  signif_genes_table.datasets[[i]]$OncodriveCLUSTL <- oncodriveCLUSTL.res.table[[i]][ 
    oncodriveCLUSTL.res.table[[i]]$SYMBOL %in%  signif_genes.datasets[[i]]$OncodriveCLUSTL , ]
  rownames(signif_genes_table.datasets[[i]]$OncodriveCLUSTL) = NULL
  
  ##### Record significant driver genes for each dataset and method
  signif_genes.methods$OncodriveCLUSTL[[i]] <- as.vector(signif_genes_table.datasets[[i]]$OncodriveCLUSTL$SYMBOL)
  
  ##### Organise the table of significant genes
  signif_genes_table.datasets[[i]]$OncodriveCLUSTL <- signif_genes_table.datasets[[i]]$OncodriveCLUSTL[ , c("SYMBOL", "ENSID", "CHROMOSOME", "SCORE", "P_EMPIRICAL", "P_VALUE_EXP", "P_ANALYTICAL", "Q_EMPIRICAL", "Q_ANALYTICAL", "LENGTH", "TOTAL_MUT", "CLUSTERED_MUT", "CLUSTERS", "SIM_CLUSTERS", "P_TOPCLUSTER", "Q_TOPCLUSTER")]
  
  #### Record significant genes and corresponding p-values (this will be used in summary)
  signif_genes_p[[i]]$oncodriveclustl <- signif_genes_table.datasets[[i]]$OncodriveCLUSTL[c("SYMBOL", "P_ANALYTICAL") ] %>%
  dplyr::rename(gene_name = SYMBOL, p_oncodriveclustl = P_ANALYTICAL)
}
```
...
all_genes_p <- vector("list", length(mafFiles))
names(all_genes_p) <- datasets.list
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(signif_genes[[i]]) > 0 ) {
    ...
    #### Merge significant genes and the corresponding p-values from different methods
    all_genes_p[[i]] <- signif_genes_p[[i]]$dndscv %>%
      dplyr::full_join(signif_genes_p[[i]]$dndsloc, by = "gene_name")
    ...
    if ( "CGI" %in% names(signif_genes.datasets[[i]]) ) {
      ...
      all_genes_p[[i]] <-all_genes_p[[i]] %>%
        dplyr::full_join(signif_genes_p[[i]]$cgi, by = "gene_name")
    }
    ...
    if ( "OncodriveFML" %in% names(signif_genes.datasets[[i]]) ) {
      ...
      all_genes_p[[i]] <-all_genes_p[[i]] %>%
        dplyr::full_join(signif_genes_p[[i]]$oncodrivefml, by = "gene_name")
    }
    
    if ( "ChasmPLUS" %in% names(signif_genes.datasets[[i]]) ) {
      signif_genes.upset.mod[[i]] <- signif_genes.upset.mod[[i]][ order( signif_genes.upset.mod[[i]][, "ChasmPLUS"], decreasing = TRUE),  ]
      all_genes_p[[i]] <-all_genes_p[[i]] %>%
        dplyr::full_join(signif_genes_p[[i]]$ChasmPLUS, by = "gene_name")
    }
    
    if ( "OncodriveCLUSTL" %in% names(signif_genes.datasets[[i]]) ) {
      signif_genes.upset.mod[[i]] <- signif_genes.upset.mod[[i]][ order( signif_genes.upset.mod[[i]][, "OncodriveCLUSTL"], decreasing = TRUE),  ]
      all_genes_p[[i]] <-all_genes_p[[i]] %>%
        dplyr::full_join(signif_genes_p[[i]]$oncodriveclustl, by = "gene_name")
    }
    ...
    if ( "ActiveDriverWGS" %in% names(signif_genes.datasets[[i]]) ) {
      ...
      all_genes_p[[i]] <-all_genes_p[[i]] %>%
        dplyr::full_join(signif_genes_p[[i]]$activedriverwgs, by = "gene_name")
    }
    ...
    #### Include p-values for significant genes from driver discovery methods
    signif_genes.upset.mod[[i]] <- signif_genes.upset.mod[[i]] %>%
      rownames_to_column("gene_name") %>%
      dplyr::full_join(all_genes_p[[i]], by = "gene_name") %>%
      dplyr::filter(!duplicated(gene_name)) %>%  # Remove duplicates
...
The estimated **θ** value(s) for substitutions for dataset(s) `r names(unlist(theta))` are: **`r unlist(theta)`**, respectively.
The estimated **θ** value(s) for indels for dataset(s) `r names(unlist( theta_indels))` are: **`r unlist( theta_indels)`**, respectively.
...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[i]]$dNdScv) > 0 ) {
    ...
    ##### Calculate x and y axis range by adding 10% of the original range
...
[ActiveDriverWGS](https://github.com/reimandlab/ActiveDriverWGSR){target="_blank"} results are **`r if ( runActiveDriverWGSChunk ) { c("AVAILABLE") } else { c("NOT AVAILABLE") }`**.
...
```{r ActiveDriverWGS_plot, comment = NA, message=FALSE, warning=FALSE, eval=runActiveDriverWGSChunk}
suppressMessages(library(plotly))
...
```{r ActiveDriverWGS_results_summary, comment = NA, message=FALSE, warning=FALSE, eval=runActiveDriverWGSChunk}
##### Present dndscv results summary table in the html report
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
Large-scale cancer sequencing studies of patient cohorts have statistically implicated many cancer driver genes, with a long-tail of infrequently mutated genes. [CHASMplus](https://karchinlab.github.io/CHASMplus/info/), a machine learning method to predict driver missense mutations, which is uniquely powered to identify rare driver mutations within the long-tail as described in the paper [ChasmPLUS](https://pubmed.ncbi.nlm.nih.gov/31202631/){target="_blank"} by Nuria Collin Tokheim *et al*, 2019.

[ChasmPLUS](https://chasmplus.readthedocs.io/en/latest/quickstart_opencravat.html){target="_blank"} results are **`r if ( runChasmplusChunk ) { c("AVAILABLE") } else { c("NOT AVAILABLE") }`**.
...
* P-value for reporting cancer driver genes: **`r params$chasmplus_p`**
* Q-value for reporting cancer driver genes: **`r params$chasmplus_q`**
...
```{r chasmPLUS_plot, comment = NA, message=FALSE, warning=FALSE, eval = runChasmplusChunk}
suppressMessages(library(plotly))
...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS) > 0 ) {
    
    ##### Avoid q-values of 0 by assigning 1*10^-16 to extremely low q-values
    q_obs <- signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS$Q_value
    q_obs[ q_obs == 0 ] <- 1*10^-4
    p_obs <- signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS$Gene_p.value
    p_obs[ p_obs == 0 ] <- 1*10^-4
    p_exp <- signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS$P_value_exp
    p_exp[ p_exp == 0 ] <- 1*10^-4
    
    ##### Prepare data frame for plotting with plotly
    data.df <- data.frame(signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS$Gene, -log10(q_obs), -log10(p_obs), -log10(p_exp), signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS$Number_of.Coding.Variants)
    colnames(data.df) <- c("gene", "q", "p_obs", "p_exp", "muts_coding_no")
    ...
    if ( !all(is.na(data.df$q)) ) {
      ...
      widges.list[[i]] <- plot_ly(data.df, x = data.df$p_exp, y = data.df$p_obs, color = data.df$q, size = data.df$muts_coding_no, text = ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
        add_text(textfont = list(size = 10), textposition = "top") %>%
...
Table(s) presenting per-dataset [ChasmPLUS](https://pubmed.ncbi.nlm.nih.gov/31202631/){target="_blank"} results, including *cluster scores*, *p-values* , *p-values* and *number of mutations in clusters*, as well as the *total number of mutations in clusters*. Cluster score of 1 means that a single hotspot hosts all observed variants.

```{r chasmplus_table, comment=NA, warning=FALSE, eval = runChasmplusChunk}
##### Generate tables with ChasmPLUS results
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
Cancer driver genes detection based on positional clustering using [OncodriveClustl](https://bbglab.irbbarcelona.org/oncodriveclustl/home){target="_blank"} is a new nucleotide sequence-based clustering algorithm to detect cancer drivers in genomic regions. OncodriveCLUSTL is based on a local background model derived from the nucleotide context mutational probabilities of the cohort under study. , as described in the paper [OncodriveClustl](https://www.ncbi.nlm.nih.gov/pubmed/23884480){target="_blank"} by [Nuria Lopez-Bigas *et al*, 2019](https://academic.oup.com/bioinformatics/article/35/22/4788/5522012?login=false){target="_blank"}.

[OncodriveClustl](https://bitbucket.org/bbglab/oncodriveclustl/src/master/){target="_blank"} results are **`r if ( runOncodriveClustlChunk ) { c("AVAILABLE") } else { c("NOT AVAILABLE") }`**.
...
* P-value for reporting cancer driver genes: **`r params$oncodriveclustl_p`**
* Q-value for reporting cancer driver genes: **`r params$oncodriveclustl_q`**
...
```{r oncodriveCLUSTL_plot, comment = NA, message=FALSE, warning=FALSE, eval = runOncodriveClustlChunk}
suppressMessages(library(plotly))
...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) > 0 ) {
    
    ##### Avoid q-values of 0 by assigning 1*10^-16 to extremely low q-values
    q_obs <- signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$Q_ANALYTICAL
    q_obs[ q_obs == 0 ] <- 1*10^-16
    p_obs <- signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$P_ANALYTICAL
    p_obs[ p_obs == 0 ] <- 1*10^-16
    p_exp <- signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$P_VALUE_EXP
    p_exp[ p_exp == 0 ] <- 1*10^-16
    
    ##### Prepare data frame for plotting with plotly
    data.df <- data.frame(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$SYMBOL, -log10(q_obs), -log10(p_obs), -log10(p_exp), signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$CLUSTERED_MUT, signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$SCORE)
    colnames(data.df) <- c("gene", "q", "p_obs", "p_exp", "muts_no", "score")
    ...
    if ( !all(is.na(data.df$q)) ) {
      ...
      widges.list[[i]] <- plot_ly(data.df, x = data.df$p_exp, y = data.df$p_obs, color = data.df$q, size = data.df$score, text = ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
        add_text(textfont = list(size = 10), textposition = "top") %>%
        add_lines(y = c(seq(min(data.df$p_obs), min(c(max(data.df$p_obs), max(data.df$p_exp))), length.out=nrow(data.df))), x = c(seq(min(data.df$p_obs), min(c(max(data.df$p_obs), max(data.df$p_exp))), length.out=nrow(data.df))), line = list(color = "grey", dash = "dash"), showlegend = FALSE, data = NULL, inherit = FALSE) %>%
        layout(title = datasets.list[i], xaxis = list(title = "-log10( expected p-values )", range = x_range), yaxis = list(title = "-log10( observed p-values )", range = y_range), margin = list(l=50, r=50, b=50, t=50, pad=4), autosize = FALSE, showlegend = FALSE, legend = list(orientation = "v", y = 0.9))
...
[Q-Q plot](https://en.wikipedia.org/wiki/Q%E2%80%93Q_plot){target="_blank"} with *y-axis* presenting -log10 of the *computed p-values* and *x-axis* the -log10 of the *expected p-values*. 

<span style="color:#ff0000">NOTE</span>, in case of very long list of detected driver genes only the most significant *computed p-values* are indicated on the Q-Q plot (check table in the *Cancer driver genes* tab to see the full list).

```{r oncodriveCLUSTL_plot_static, comment = NA, message=FALSE, warning=FALSE, results="asis", eval=runOncodriveClustlChunk}
for ( i in 1:length(mafFiles) ) {
  
  ##### Check if the OncodriveCLUSTL results plot exists
  if ( file.exists(paste0(dirname(oncodriveCLUSTL.res[i]), "/quantile_quantile_plot.png")) ) {
    
    ##### Read in the oncodriveFML PNG files
    cat(paste("\n\n <b>", datasets.list[i], "</b> \n\n", sep=" "))
    cat("![](",paste(paste0(dirname(oncodriveCLUSTL.res[i]), "/quantile_quantile_plot.png")),")")
    cat("<br/><br/><br/>")

    ##### Copy the OncodriveCLUSTL results plot (HTML and PNG) to the report folder
    file.copy(paste0(dirname(oncodriveCLUSTL.res[i]), "/quantile_quantile_plot.png"), paste0(outDir, "/driver_analysis_oncodriveCLUSTL_plot_", datasets.list[i], ".png"), overwrite = TRUE, copy.mode = TRUE, copy.date = FALSE)
    file.copy(paste0(oncodriveCLUSTL.res[i], ".html"), paste0(outDir, "/driver_analysis_oncodriveCLUSTL_plot_", datasets.list[i], ".html"), overwrite = TRUE, copy.mode = TRUE, copy.date = FALSE)
    
  } else {
    cat(paste("OncodriveCLUSTL results plot \"", paste0(oncodriveCLUSTL.res[i], ".png"), "\" is not present for dataset", datasets.list[i], "\n\n", sep=" "))
  }
}
```
...
Table(s) presenting per-dataset [OncodriveClustl](https://www.ncbi.nlm.nih.gov/pubmed/23884480){target="_blank"} results, including *cluster scores*, *p-values* , *p-values* and *number of mutations in clusters*, as well as the *total number of mutations in clusters*. Cluster score of 1 means that a single hotspot hosts all observed variants.

```{r oncodriveCLUSTL_table, comment=NA, warning=FALSE, eval = runOncodriveClustlChunk}
##### Generate tables with oncodriveCLUSTL results
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Check if the field with AA change info is present in given MAF at all
  if ( length(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) != 0 ) {
    
    #### Check if the any of the mutated genes demonstrates significant cluster score
    if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) > 0 ) {
      
      widges.list[[i]] <- DT::datatable(data = signif_genes_table.datasets[[i]]$OncodriveCLUSTL, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 2), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE ) %>%
        DT::formatStyle( columns = names(signif_genes_table.datasets[[i]]$OncodriveCLUSTL), 'text-align' = 'center' ) %>%
    formatRound(columns = c("SCORE"), 1) %>%
    formatRound(columns = c("P_EMPIRICAL", "P_VALUE_EXP", "P_ANALYTICAL", "Q_EMPIRICAL", "Q_ANALYTICAL", "P_TOPCLUSTER", "Q_TOPCLUSTER"), 6)
...
```{r pchange_field_check, comment = NA, message=FALSE, warning=FALSE, eval = FALSE}
...
```{r prot_structre, comment = NA, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r lollipop_plots_dNdScv, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results="asis", eval=FALSE}
...
```{r lollipop_plots_dNdSloc, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results="asis", eval=FALSE}
##### Generate lollipop plot for each dataset for top mutated genes
if ( pchangeStatus ) {
...
```{r lollipop_plots_ActiveDriverWGS, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results="asis", eval = runActiveDriverWGSChunk}
##### Generate lollipop plot for each dataset for top mutated genes
if ( pchangeStatus ) {
...
`r if ( runChasmplusChunk ) { c("### ChasmPLUS {.tabset}") } else { c(" ") }`

```{r lollipop_plots_ChasmPLUS, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results="asis", eval = FALSE}
##### Generate lollipop plot for each dataset for top mutated genes
if ( pchangeStatus ) {
  
  output_plot <- list()
  genes <- unique(verify_genes(unlist(signif_genes.methods$ChasmPLUS)))
  ...
  for( i in 1:length(genes) ){
    ...
    if ( nrow(gff[HGNC %in% genes[i]]) != 0 ) {
      ...
      cat(paste("The protein structure for protein encoded by", unlist(signif_genes.methods$ChasmPLUS)[i], "is not available\n\n", sep=" "))
      cat("\n***\n")
    }
  }
...
`r if ( runOncodriveClustlChunk ) { c("### OncodriveCLUSTL {.tabset}") } else { c(" ") }`

```{r lollipop_plots_OncodriveCLUSTL, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results="asis", eval = FALSE}
##### Generate lollipop plot for each dataset for top mutated genes
if ( pchangeStatus ) {
  
  output_plot <- list()
  genes <- unique(verify_genes(unlist(signif_genes.methods$OncodriveCLUSTL)))
  ...
  for( i in 1:length(genes) ){
    ...
    if ( nrow(gff[HGNC %in% genes[i]]) != 0 ) {
      ...
      cat(paste("The protein structure for protein encoded by", unlist(signif_genes.methods$OncodriveCLUSTL)[i], "is not available\n\n", sep=" "))
      cat("\n***\n")
    }
  }
...
```{r lollipop_plots_OncodriveFML, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results="asis", eval = FALSE}
##### Generate lollipop plot for each dataset for top mutated genes
if ( pchangeStatus ) {
...
```{r mutation_annotation_tabs, comment = NA, message=FALSE, warning=FALSE}
# Create a list to store the subset results
l1 <- list()

# Iterate over each element of signif_genes.methods
for (i in 1:length(mafFiles)) {
  for (j in 1:length(signif_genes.methods)) {
    # Apply subset condition and store the result in the l1 list
    l1[[j]] <-
      dt_view(dndsout[[i]]$annotmuts[dndsout[[i]]$annotmuts$gene %in% signif_genes.methods[[j]][[i]],])
  }
}

# Set names of the subset_results list
names(l1) <- names(signif_genes.methods)
...
div(
  ...
  tags$ul(
    ...
    lapply(
      ...
      function(nm){
        tags$li(
          class=ifelse(nm == names(l1)[i], "active", ""),
          tags$a(
            "data-toggle" = "tab",
            "role" = "tab",
            href = paste0("#tab1-", nm),
    ...
    lapply(
      ...
      function(nm){
        tags$div(
          ...
          class=ifelse(nm == names(l1)[i], "tab-pane active", "tab-pane"),
          #  id will need to match the id provided to the a href above
          id = paste0("tab1-", nm),
          plotly::as_widget(l1[[nm]])
        )
      }
    )
  )
...
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
```{r details_mut_ActiveDriverWGS_nonsynon, comment = NA, message=FALSE, warning=FALSE, eval= runActiveDriverWGSChunk}
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
```{r details_mut_ActiveDriverWGS_silent, comment = NA, message=FALSE, warning=FALSE, eval= runActiveDriverWGSChunk}
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
`r if ( runChasmplusChunk ) { c("***") } else { c(" ") }`

`r if ( runChasmplusChunk ) { c("### ChamPLUS {.tabset .tabset-fade}") } else { c(" ") }`

`r if ( runChasmplusChunk ) { c("#### Non-synonymous") } else { c(" ") }`

```{r details_mut_ChasmPLUS_nonsynon, comment = NA, message=FALSE, warning=FALSE, eval = runChasmplusChunk}
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  if ( length(signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS) != 0 ) {
    widges.list[[i]] <- mut.details.datasets(mafInfo[i], datasets.list[i], signif_genes.methods$ChasmPLUS[[i]], type = "nonsynonymous")
  } else {
    cat(paste("This section was skipped for dataset", datasets.list[i], "since the corresponding MAF does not contain field with amino acid changes details!\n\n", sep=" "))
  }
}
...
`r if ( runChasmplusChunk ) { c("#### Silent") } else { c(" ") }`

```{r details_mut_ChasmPLUS_silent, comment = NA, message=FALSE, warning=FALSE, eval = runChasmplusChunk}
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  if ( length(signif_genes_table.datasets[[datasets.list[i]]]$ChasmPLUS) != 0 ) {
    widges.list[[i]] <- mut.details.datasets(mafInfo[i], datasets.list[i], signif_genes.methods$ChasmPLUS[[i]], type = "silent")
  } else {
    cat(paste("This section was skipped for dataset", datasets.list[i], "since the corresponding MAF does not contain field with amino acid changes details!\n\n", sep=" "))
  }
}
...
`r if ( runOncodriveClustlChunk ) { c("***") } else { c(" ") }`

`r if ( runOncodriveClustlChunk ) { c("### OncodriveCLUSTL {.tabset .tabset-fade}") } else { c(" ") }`

`r if ( runOncodriveClustlChunk ) { c("#### Non-synonymous") } else { c(" ") }`

```{r details_mut_OncodriveCLUSTL_nonsynon, comment = NA, message=FALSE, warning=FALSE, eval = runOncodriveClustlChunk}
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  if ( length(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) != 0 ) {
    widges.list[[i]] <- mut.details.datasets(mafInfo[i], datasets.list[i], signif_genes.methods$OncodriveCLUSTL[[i]], type = "nonsynonymous")
  } else {
    cat(paste("This section was skipped for dataset", datasets.list[i], "since the corresponding MAF does not contain field with amino acid changes details!\n\n", sep=" "))
  }
}
...
`r if ( runOncodriveClustlChunk ) { c("#### Silent") } else { c(" ") }`

```{r details_mut_OncodriveCLUSTL_silent, comment = NA, message=FALSE, warning=FALSE, eval = runOncodriveClustlChunk}
##### Provide detailed information for each dataset for user-defined number of top mutated genes
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  if ( length(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) != 0 ) {
    widges.list[[i]] <- mut.details.datasets(mafInfo[i], datasets.list[i], signif_genes.methods$OncodriveCLUSTL[[i]], type = "silent")
  } else {
    cat(paste("This section was skipped for dataset", datasets.list[i], "since the corresponding MAF does not contain field with amino acid changes details!\n\n", sep=" "))
  }
}
...
```{r circos_prep, comment = NA, message=FALSE, warning=FALSE, results="asis", eval=FALSE}
...
```{r circos_plot, comment = NA, message=FALSE, warning=FALSE, results="asis", eval=FALSE}
