...
lollipops.datasets <- function(mafInfo, datasets, gene) {
  ...
  for ( dataset in datasets ) {
    ...
    ##### Check if the gene has any mutations in corresponding dataset
    if ( nrow(mut) != 0 ) {
...
lollipops.methods <- function(pchangeStatus, mafInfo, datasets, signif_genes.met) {
  ##### Generate lollipop plot for each dataset for top mutated genes
if ( pchangeStatus ) {
  
  output_plot <- list()
  genes <- unique(verify_genes(unlist(signif_genes.met)))
  ...
  for( i in 1:length(genes) ){
    ...
    if ( nrow(gff[HGNC %in% genes[i]]) != 0 ) {
      ...
      cat(paste("The protein structure for protein encoded by", unlist(signif_genes.met)[i], "is not available\n\n", sep=" "))
      cat("\n***\n")
    }
  }
...
Tables with detailed information for nonsynonymous variants in affected samples, as provided in corresponding MAF file(s), for cancer driver genes detected by the following methods: **`r paste(methods, collapse = ", ")`**.
...
```{r mutation_details_tabs, comment = NA, message=FALSE, warning=FALSE}
# Create a list to store the subset results
l2 <- list()
...
for (i in 1:length(mafFiles)) {
  for (j in 1:length(signif_genes.methods)) {
    # Apply subset condition and store the result in the l2 list - only displaying nonsynonymous for now (other option for example is - silent)
    if ( length(signif_genes_table.datasets[[datasets.list[i]]][[j]]) != 0 ) {
      l2[[j]] <- dt_view(mut.details.datasets(mafInfo[i], datasets.list[i], signif_genes.methods[[j]][[i]], type = "nonsynonymous"))
    }
  }
}

# Set names of the subset_results list
names(l2) <- names(signif_genes.methods)
...
div(
  ...
  tags$ul(
    ...
    lapply(
      ...
      function(nm){
        tags$li(
          class=ifelse(nm == names(l2)[i], "active", ""),
          tags$a(
            "data-toggle" = "tab",
            "role" = "tab",
            href = paste0("#tab1-", nm),
  ...
  tags$div(
    ...
    lapply(
      ...
      function(nm){
        tags$div(
          #  make the first tabpane active
          class=ifelse(nm == names(l2)[i], "tab-pane active", "tab-pane"),
          #  id will need to match the id provided to the a href above
          id = paste0("tab1-", nm),
          plotly::as_widget(l2[[nm]])
        )
      }
    )
  )
