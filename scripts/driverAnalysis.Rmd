...
* **[ActiveDriverWGS](https://github.com/reimandlab/ActiveDriverWGSR){target="_blank"}** (see paper by [Helen et al., 2020](https://pubmed.ncbi.nlm.nih.gov/31954095){target="_blank"} for details)
...
* **activedriver_p**: `r params$activedriver_p`
* **activedriver_fdr**: `r params$activedriver_fdr`
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( is.na(match("chr", dndscvInfo[[i]]$chr)) ) {
...
##### Convert the MAF calss objest to the format relevant for ActiveDriverWGS
##### NOTE, silent mutations are also included
ActiveDriverInfo <- vector("list", length(mafFiles))
names(ActiveDriverInfo) <- datasets.list

for ( i in 1:length(mafFiles) ) {
  
  ActiveDriverInfo[[i]] <- rbind(mafInfo[[i]]@data[, c("SYMBOL", "Chromosome", "Start_Position", "End_Position", "Reference_Allele", "Tumor_Seq_Allele2", "Tumor_Sample_Barcode")], mafInfo[[i]]@maf.silent[, c("SYMBOL", "Chromosome", "Start_Position", "End_Position", "Reference_Allele", "Tumor_Seq_Allele2", "Tumor_Sample_Barcode")])
  names(ActiveDriverInfo[[i]]) <- c("SYMBOL", "chr", "pos1", "pos2", "ref", "alt", "patient")
  ActiveDriverInfo[[i]]$patient <- as.vector(ActiveDriverInfo[[i]]$patient)
  
  ##### Run analysis for selected genes
  if ( params$genes_list != "none" ){
      ActiveDriverInfo[[i]] <- ActiveDriverInfo[[i]][ ActiveDriverInfo[[i]]$SYMBOL %in% goi,]
  }
  
  ##### Remove gene column
  ActiveDriverInfo[[i]] <- ActiveDriverInfo[[i]][ , -c("SYMBOL")]
  
  ##### Make sure that the chromosome numbers start with "chr"
  if ( is.na(match("chr", ActiveDriverInfo[[i]]$chr)) ) {
    ActiveDriverInfo[[i]]$chr <- paste0("chr", ActiveDriverInfo[[i]]$chr)
  }
  
  ##### Make sure that sex chromosomes are defined as chrX and chrY
  ActiveDriverInfo[[i]]$chr <- gsub("chr23", "chrX", ActiveDriverInfo[[i]]$chr)
  ActiveDriverInfo[[i]]$chr <- gsub("chr24", "chrY", ActiveDriverInfo[[i]]$chr)
  
  ##### Make sure that only autosomal and sex chromosomes are provided
  ActiveDriverInfo[[i]] <- ActiveDriverInfo[[i]][ ActiveDriverInfo[[i]]$chr %in% c(paste0("chr", 1:22), "chrX", "chrY"), ]
  
  ##### Finally make sure that all rows have defined "ref" and "alt" values (none NAs)
  ActiveDriverInfo[[i]] <- ActiveDriverInfo[[i]][complete.cases(ActiveDriverInfo[[i]]), ]
}
...
##### Run analysis for selected genes
if ( params$genes_list != "none" ){
    elements = elements[ elements$id %in% goi,]
}
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  ActiveDriverWGS.res[[i]] = suppressWarnings(ActiveDriverWGS(mutations = ActiveDriverInfo[[i]], elements = elements, sites = NULL, window_size = 50000, filter_hyper_MB = params$hypermut_sample_cutoff/100, recovery.dir = NULL, mc.cores = 1, ref_genome = paste0("hg", params$ucsc_genome_assembly)))
  ...
  signif_genes.datasets[[i]]$ActiveDriverWGS <- unique(c(ActiveDriverWGS.res[[i]]$id[ ActiveDriverWGS.res[[i]]$element_enriched & ActiveDriverWGS.res[[i]]$pp_element <= params$activedriver_p  ], ActiveDriverWGS.res[[i]]$id[ ActiveDriverWGS.res[[i]]$element_enriched & ActiveDriverWGS.res[[i]]$fdr_element <= params$activedriver_fdr ]))
...
* P-value threshold for reporting cancer driver genes: **`r params$activedriver_p`**
* False discovery rate (FDR) threshold for reporting cancer driver genes: **`r params$activedriver_fdr`**
...
Table(s) presenting per-dataset [ActiveDriverWGS](https://github.com/reimandlab/ActiveDriverWGSR){target="_blank"} results, including *p-values*, *false discovery rate (FDR) values*, *number of mutations in individual samples*, as well as the *expected number of mutations in individual samples*.
