...
for ( i in 1:length(mafFiles) ) {
  ...
  signif_genes_cv[[i]] <- sel_cv[[i]][sel_cv[[i]]$qglobal_cv < params$dnds_q, ]
  ...
  signif_genes_loc[[i]] <- dndsout[[i]]$sel_loc[ dndsout[[i]]$sel_loc$qall_loc < params$dnds_q, ]
...
##### Create list to hold dataframes with sinigficant driver genes
...
<details>
<summary>Report input parameters</summary>
<font size="2">
* Genome assembly version: **`r params$ucsc_genome_assembly`**
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Check if the OncodriveFML results plot (HTML) exists
  if ( nrow(oncodriveFML.res.table.signif[[i]]) > 0 ) {
...
Table(s) with detailed annotation of **coding mutations** (detected by the following methods: **`r paste(names(signif_genes.datasets[[i]]), collapse = ", ")`**) in affected samples. **Note**, mutations detected in non-coding genomic regions, e.g. those shortlisted by *OncodriveFML*, are not listed in the table below.
