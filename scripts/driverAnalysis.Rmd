...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(signif_genes_table.datasets[[i]]$dNdScv) > 0 ) {
    ...
    data.df <- data.frame(signif_genes_table.datasets[[i]]$dNdScv$gene_name, q_global, mutations_no, p_subs, p_ind )
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(signif_genes_table.datasets[[i]]$dNdSloc) > 0 ) {
    ...
    ##### Prepare data frame for plotting with plotly
    data.df <- data.frame(signif_genes_table.datasets[[i]]$dNdSloc$gene_name, q_all, mutations_no, q_mis )
