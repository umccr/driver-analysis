...
for ( i in 1:length(mafFiles) ) {
  ...
  oncodriveCLUSTL.res.table[[i]] <- oncodriveCLUSTL.res.table[[i]][ order(oncodriveCLUSTL.res.table[[i]]$P_EMPIRICAL), ]
  rownames(oncodriveCLUSTL.res.table[[i]]) = NULL
  
  ##### Calculate expected p-value
  oncodriveCLUSTL.res.table[[i]]$P_VALUE_EXP <- ppoints(length(oncodriveCLUSTL.res.table[[i]]$P_EMPIRICAL))

  ##### Record significant driver genes for each dataset
  signif_genes.datasets[[i]]$OncodriveCLUSTL <- as.character(oncodriveCLUSTL.res.table[[i]][
    oncodriveCLUSTL.res.table[[i]]$P_EMPIRICAL <= params$oncodriveclustl_p &
      oncodriveCLUSTL.res.table[[i]]$Q_EMPIRICAL <= params$oncodriveclustl_q, "SYMBOL"])
...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) > 0 ) {
    ...
    q_obs <- signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$Q_EMPIRICAL
    q_obs[ q_obs == 0 ] <- 1*10^-16
    p_obs <- signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$P_EMPIRICAL
