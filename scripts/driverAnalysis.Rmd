...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) > 0 ) {
    ...
    ##### Prepare data frame for plotting with plotly
    data.df <- data.frame(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$SYMBOL, -log10(q_obs), -log10(p_obs), -log10(p_exp), signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL$TOTAL_MUT )
