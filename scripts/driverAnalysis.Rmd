...
if ( length(unlist(exclsamples)) > 0 ) {
...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$CGI) > 0 ) {
    ...
    ##### Generate scatter-plot
    widges.list[[i]] <- plot_ly(data.df, x = ~Frequency, y = ~Deleterious, color = ~Consequence, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 600) %>%
