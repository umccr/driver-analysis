...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( params$genes_list != "none" ){
    ...
    if ( params$ucsc_genome_assembly == 19 ) {
      ...
      dndsout[[i]] <- dndscv(dndscvInfo[[i]], gene_list = goi, refdb = paste0("hg", as.character(params$ucsc_genome_assembly)), sm = "192r_3w", kc = "cgc81", cv = NULL, max_muts_per_gene_per_sample = params$max_muts_per_gene, max_coding_muts_per_sample = params$hypermut_sample_cutoff, use_indel_sites = TRUE, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = TRUE, outp = 3)
    }
  ...
  ##### Run analysis for all genes
    ...
    if ( params$ucsc_genome_assembly == 19 ) {
      ...
      dndsout[[i]] <- dndscv(dndscvInfo[[i]], gene_list = RefCDSgenes, refdb = paste0("hg", as.character(params$ucsc_genome_assembly)), sm = "192r_3w", kc = "cgc81", cv = NULL, max_muts_per_gene_per_sample = params$max_muts_per_gene, max_coding_muts_per_sample = params$hypermut_sample_cutoff, use_indel_sites = TRUE, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = TRUE, outp = 3)
    }
...
`r if ( length(unlist(hypermut_samples)) > 0 ) { c("***") }`

`r if ( length(unlist(hypermut_samples)) > 0 ) { c("#### Hypermutator samples") }`

`r if ( length(unlist(hypermut_samples)) > 0 ) { c(paste0("Table(s) listing sample(s) with >",  params$hypermut_sample_cutoff, " coding mutations. These have been removed from the analyses to protect against loss of sensitivity from ultra-hypermutators.")) }`
...
*X-axis* indicates the *number of variants* observed in individual genes and *y-axis* illustrates *p-values* (in -log10 scale) based on all non-synonymous substitutions. The *size* of the points is proportional to -log10 *p-values* based exclusively on missense substitutions detected in individual genes.
...
for ( i in 1:length(mafFiles) ) {
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$CGI) > 0 ) {
    ...
    ##### Prepare data frame for plotting with plotly
    data.df <- data.frame(freq, deleterious, consequence )
    colnames(data.df) <- c("Gene","Frequency", "Deleterious", "Consequence")
    
    ##### Generate scatter-plot
    widges.list[[i]] <- plot_ly(data.df, x = ~Frequency, y = ~Deleterious, text = ~ Gene, color = ~Consequence, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 600) %>%
