...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( params$genes_list != "none" ){
    ...
    if ( params$ucsc_genome_assembly == 19 ) {
      ...
      dndsout[[i]] <- dndscv(dndscvInfo[[i]], refdb = load(file=params$refdb_38), sm = "192r_3w", kc = "cgc81", cv = NULL, max_muts_per_gene_per_sample = params$max_muts_per_gene, max_coding_muts_per_sample = params$hypermut_sample_cutoff, use_indel_sites = TRUE, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = TRUE, outp = 3)
    }
