...
* **[CHASMPlus](https://chasmplus.readthedocs.io/en/latest/quickstart_opencravat.html#interpretation){target="_blank"}**  (see paper by [Collin Tokheim *et al*., 2019](https://pubmed.ncbi.nlm.nih.gov/31202631/){target="_blank"} for details - WIP)
...
```{r mutations_annotation_ActiveDriverWGS, comment = NA, message=FALSE, warning=FALSE, eval = runActiveDriverWGSChunk}
##### Present results summary table in the html report
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
`r if ( runOncodriveClustlChunk ) { c("### OncodriveCLUSTL") } else { c(" ") }`

```{r mutations_annotation_OncodriveCLUSTL, comment = NA, message=FALSE, warning=FALSE, eval = runOncodriveClustlChunk}
##### Present results summary table in the html report
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  if ( length(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUSTL) != 0 ) {
    ...
    annotmuts <- dndsout[[i]]$annotmuts[ dndsout[[i]]$annotmuts$gene %in% signif_genes.methods$OncodriveCLUSTL[[i]], ]
