...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Report the sinigficant driver genes
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( file.exists(paste0(oncodriveFML.res[i], ".html")) ) {
    ...
    oncodriveFML.res.table.signif <- oncodriveFML.res.table.signif[[i]]
...
Tables with detailed information in affected samples, as as provided in corresponding MAF file(s), for cancer driver genes detected by the following methods: **`r paste(names(signif_genes.datasets[[i]]), collapse = ", ")`**.
...
<details>
<summary>Non-synonymous variants definition</summary>
<font size="2">

**Non-synonymous variants** are defined as variants with the following consequences: *`r params$nonSyn_list`*.
...
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$dNdScv[[i]], type = "all")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
<details>
<summary>Sileny variants definition</summary>
<font size="2">

**Sileny variants** are defined as variants with the following consequences: *`r unique(mafInfo[[dataset]]@maf.silent$Variant_Classification)`*.
...
```{r details_mut_dNdSloc, comment = NA, message=FALSE, warning=FALSE}
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$dNdSloc[[i]], type = "all")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
```{r details_mut_OncodriveCLUST, comment = NA, message=FALSE, warning=FALSE}
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$OncodriveCLUST[[i]], type = "all")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
```{r mutations_annotation_OncodriveFML, comment = NA, message=FALSE, warning=FALSE, eval=runOncodrivefmlChunk}
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$OncodriveFML[[i]], type = "all")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
