...
mut.details.datasets <- function(mafInfo, datasets, genes, type = "nonsynonymous") {
  ...
  if ( length(datasets.noMut) != 0 ) {
    ...
    cat(paste("Note, queried gene(s) have no non-synonymous variants reported in the following dataset(s):", paste(datasets.noMut, collapse = ", "), "\n\n\n\n", sep=" "))
  }
...
if ( any(!is.na(params$oncodrivefml)) ){
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( params$ratios_ci ) {
    ...
    ##### Organise the table of significant genes
    signif_genes_table.datasets[[i]]$dNdScv <- signif_genes_table.datasets[[i]]$dNdScv[ , c("gene_name", "pglobal_cv", "qglobal_cv", "n_syn", "n_mis", "n_non", "n_spl", "n_ind", "wmis_cv", "mis_low", "mis_high", "pmis_cv", "qmis_cv", "wnon_cv", "wspl_cv", "tru_mle", "tru_low", "tru_high", "ptrunc_cv", "qtrunc_cv", "pallsubs_cv", "qallsubs_cv", "wind_cv", "pind_cv")]
    
  } else {
    
    ##### Organise the table of significant genes
    signif_genes_table.datasets[[i]]$dNdScv <- signif_genes_table.datasets[[i]]$dNdScv[ , c("gene_name", "pglobal_cv", "qglobal_cv", "n_syn", "n_mis", "n_non", "n_spl", "n_ind", "wmis_cv", "pmis_cv", "qmis_cv", "wnon_cv", "wspl_cv", "ptrunc_cv", "qtrunc_cv", "pallsubs_cv", "qallsubs_cv", "wind_cv", "pind_cv")]
  }
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Record sinigficant driver genes for each dataset and method
  signif_genes.methods$OncodriveFML[[i]] <- signif_genes_table.datasets[[i]]$OncodriveFML$SYMBOL
...
NOTE: **Only non-synonymous variants** are used for this analysis.
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(signif_genes_table.datasets[[i]]$dNdScv) > 0 ) {
    ...
    p <- plot_ly(data.df, x = ~mutations_no, y = data.df$q_global, color = data.df$p_ind, size = data.df$p_subs, text = ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(signif_genes_table.datasets[[i]]$dNdSloc) > 0 ) {
    ...
    ##### Generate scatter-plot
    p <- plot_ly(data.df, x = ~mutations_no, y = data.df$q_all, size = data.df$q_mis, text= ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
      add_text(textfont = list(size = 10), textposition = "top") %>%
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( !is.null(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUST) ) {
    ...
    if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveCLUST) > 0 ) {
      ...
      ##### Generate scatter-plot
      p <- plot_ly(data.df, x = ~mutations_no, y = data.df$fdr, color = data.df$clusterScores, size = data.df$clusters, text= ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
        add_text(textfont = list(size = 10), textposition = "top") %>%
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(oncodriveFML.res.table[[datasets.list[i]]]) > 0 ) {
    ...
    ##### Generate scatter-plot
    p <- plot_ly(data.df, x = data.df$p_exp, y = data.df$p_obs, color = data.df$q, size = data.df$muts_no, text = ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
    ...
    ##### Save the plot as html (PLOTLY)
    htmlwidgets::saveWidget(as_widget(p), paste0(outDir, "/driver_analysis_oncodriveFML_plotly_", datasets.list[i], ".html"), selfcontained = TRUE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(signif_genes_table.datasets[[datasets.list[i]]]$OncodriveFML) > 0 ) {
    ...
    if ( !all(is.na(data.df$q)) ) {
      ...
      p <- plot_ly(data.df, x = data.df$p_exp, y = data.df$p_obs, size = data.df$muts_no, text = ~gene, mode = "markers",  marker = list(opacity = 0.8, line = list(color = "black", width = 1)), width = 800, height = 400) %>%
        add_text(textfont = list(size = 10), textposition = "top") %>%
        add_lines(y = c(seq(min(data.df$p_obs), min(c(max(data.df$p_obs), max(data.df$p_exp))), length.out=nrow(data.df))), x = c(seq(min(data.df$p_obs), min(c(max(data.df$p_obs), max(data.df$p_exp))), length.out=nrow(data.df))), line = list(color = "grey", dash = "dash"), showlegend = FALSE, data = NULL, inherit = FALSE) %>%
...
NOTE: mutations detected in non-coding genomic regions, e.g. those shortlisted by *OncodriveFML*, are not listed in the table(s) below.
...
Tables with detailed variant's information in affected samples, as as provided in corresponding MAF file(s), for cancer driver genes detected by the following methods: **`r paste(names(signif_genes.datasets[[i]]), collapse = ", ")`**.
...
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$dNdScv[[i]], type = "nonsynonymous")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$dNdScv[[i]], type = "silent")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$dNdSloc[[i]], type = "nonsynonymous")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$OncodriveCLUST[[i]], type = "nonsynonymous")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$OncodriveCLUST[[i]], type = "silent")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$OncodriveFML[[i]], type = "nonsynonymous")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, signif_genes.methods$OncodriveFML[[i]], type = "silent")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
